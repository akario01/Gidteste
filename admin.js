/* ===================================================================
   PARTE 1 DE 2 DO ARQUIVO JAVASCRIPT (JS) V4.2 - VERSÃO FINAL TESTADA
   =================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
                ...
                agendamentosReaisFuturos.forEach(doc => {
                    ...
                    card.innerHTML = `
                        <div class="appointment-time-block">
                            <div class="time">${app.horario}</div>
                            <div class="date">${formattedDate}</div>
                        </div>
                        <div class="appointment-details-block">
                            <div class="info">
                                <strong class="client-name">${app.nomeCliente}</strong>
                                <span class="service-name">${app.servicoNome}</span>
                            </div>
                            <div class="actions">
        ...
        agendamentosDeHojeParaLembrar.forEach(app => {
            ...
            card.innerHTML = `
                ...
                <div class="appointment-details-block">
                    ...
                    <div class="actions">
                        <a href="${whatsappUrl}" target="_blank" class="reminder-btn" title="Enviar lembrete via WhatsApp">
    ...
    async function loadAgendaGrid(selectedDate) {
        ...
        while (horarioAtual < fim) {
            ...
            if (agendamento) {
                if (agendamento.isBlock) {
                    slotDiv.innerHTML = `<div class="time-label">${timeStr}</div><div class="blocked-slot" data-id="${agendamento.id}">Bloqueado</div>`;
                    slotDiv.addEventListener('click', (e) => showActionMenu(e.currentTarget, 'blocked', selectedDate, timeStr));
                } else {
                    slotDiv.innerHTML = `
                    ...
                    slotDiv.addEventListener('click', (e) => showActionMenu(e.currentTarget, 'appointment', selectedDate, timeStr, agendamento.id));
                }
            } else {
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                const blockData = {
                    ...
                    horario: time,
                    isBlock: true,
                    servicoNome: "Bloqueado pelo Administrador",
                    timestamp: firebase.firestore.FieldValue.serverTimestamp()
                };
                await db.collection('agendamentos').add(blockData);
            }
            loadAgendaGrid(date);
            loadDashboardData();
        } catch (error) {
    ...
    function showActionMenu(slotElement, type, date, time, appointmentId = null) {
        ...
        switch (type) {
            case 'empty':
                menuContent = `
                    <button data-action="book"><i class="fas fa-plus"></i> Agendar Cliente</button>
                    <button data-action="block"><i class="fas fa-lock"></i> Bloquear Horário</button>
                `;
                break;
            case 'blocked':
                menuContent = `<button data-action="unblock"><i class="fas fa-unlock"></i> Desbloquear Horário</button>`;
                break;
            case 'appointment':
                menuContent = `<button data-action="delete"><i class="fas fa-trash"></i> Excluir Agendamento</button>`;
                break;
        }
        ...
        menu.addEventListener('click', (e) => {
            ...
            if (action) {
                switch (action) {
                    case 'book':
                        initManualBookingModal(db, date, () => loadAgendaGrid(date), time);
                        break;
                    case 'block':
                        if (confirm(`Deseja bloquear o horário das ${time}?`)) {
                            toggleBlockTime(date, time);
                        }
                        break;
                    case 'unblock':
                    ...
                    case 'delete':
                        if (confirm('Tem certeza que deseja excluir este agendamento?')) {
                            db.collection('agendamentos').doc(appointmentId).delete().then(() => {
                                loadAgendaGrid(date);
                                loadDashboardData();
                            });
                        }
    ...
    function initManualBookingModal(db, selectedDate, onSaveCallback, preselectedTime = null) {
        ...
        form.onsubmit = async (e) => {
            ...
            const clientName = form['manual-client-name'].value;
            const selectedTime = form['manual-time-picker'].value;
            const data = { nomeCliente: clientName, telefoneCliente: `55${form['manual-client-phone'].value.replace(/\D/g, '')}`, data: selectedDate, horario: selectedTime, servicoNome: service.nome, servicoPreco: service.preco, duracao: service.duracao, timestamp: firebase.firestore.FieldValue.serverTimestamp() };
            
            await db.collection('agendamentos').add(data);
            
            const formattedDate = new Date(selectedDate + 'T00:00:00').toLocaleDateString('pt-BR');
            const message = `Novo Agendamento (Manual)!\n\n*Cliente:* ${clientName}\n*Data:* ${formattedDate}\n*Horário:* ${selectedTime}\n*Serviço:* ${service.nome}`;
...
// FIM DA PARTE 1
/* ===================================================================
   PARTE 2 DE 2 DO ARQUIVO JAVASCRIPT (JS) V4.2 - VERSÃO FINAL TESTADA
   =================================================================== */

    // ✅✅✅ FUNÇÃO ATUALIZADA (V4.2) - CORREÇÃO FINAL E TESTADA ✅✅✅
    async function initClientProfileModal(db, phone, clientName, isNewClient = false, onSaveCallback) {
        const modal = document.getElementById('client-profile-modal-backdrop');
        const form = document.getElementById('client-profile-form');
        const formFieldsContainer = document.getElementById('client-form-fields');
        ...
        formFieldsContainer.innerHTML = '<div class="loading-spinner"></div>';
        ...
        const renderForm = (clientData) => {
            clientData.preferencias = clientData.preferencias || {};
            formFieldsContainer.innerHTML = `
                ...
                <div class="input-group"><label for="profile-obs">Observações</label><textarea id="profile-obs" class="input-field">${clientData.preferencias.observacoes || ''}</textarea></div>
            `;
        };
        ...
        const loadHistory = async (clientPhone) => {
            ...
            try {
                ...
                const snapshot = await db.collection('agendamentos')
                    .where('telefoneCliente', '==', clientPhone)
                    .where('data', '<=', today)
                    .orderBy('data', 'desc')
                    .get();

                let historyHTML = '<h3>Histórico de Visitas</h3>';
                if (snapshot.empty) {
                ...
                snapshot.forEach(doc => {
                    ...
                    if (!app.isBlock) {
                        visitCount++;
                        const price = parseFloat(app.servicoPreco.replace(',', '.')) || 0;
                        totalSpent += price;
                        const formattedDate = new Date(app.data + 'T00:00:00').toLocaleDateString('pt-BR');
                        historyList.push(`<li>${formattedDate} - ${app.servicoNome} (R$ ${app.servicoPreco})</li>`);
                    }
                });

                if (visitCount > 0) {
                    historyHTML += `<ul class="client-history-list">${historyList.join('')}</ul>`;
                    historyHTML += `<div class="client-history-summary">
                                        <strong>Total de Visitas:</strong> ${visitCount}<br>
                                        <strong>Total Gasto:</strong> ${totalSpent.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                   </div>`;
                    historyHTML += `<button type="button" id="clear-history-btn" class="btn-secondary">Limpar Histórico</button>`;
                } else {
                    historyHTML += '<p>Nenhum histórico de agendamentos encontrado.</p>';
                }
                ...
                if (clearHistoryBtn) {
                    clearHistoryBtn.addEventListener('click', async () => {
                        if (confirm(`Tem certeza que deseja apagar TODO o histórico de agendamentos de ${clientName}? Esta ação não pode ser desfeita.`)) {
                            const batch = db.batch();
                            snapshot.forEach(doc => {
                                batch.delete(doc.ref);
                            });
                            await batch.commit();
                            loadHistory(clientPhone);
                            loadDashboardData();
                        }
                    });
                }
                ...
                historyContainer.innerHTML = '<h3>Histórico de Visitas</h3><p>Ocorreu um erro ao carregar o histórico. Verifique se o índice do Firebase foi criado corretamente.</p>';
            }
        };

        if (isNewClient) {
            renderForm({});
            historyContainer.innerHTML = '<h3>Histórico de Visitas</h3><p>Novo cliente, nenhum histórico disponível.</p>';
        } else {
        ...
        form.onsubmit = async (e) => {
            ...
            const dataToSave = {
                ...
                telefone: finalPhone,
                preferencias: {
                    observacoes: document.getElementById('profile-obs').value
                },
